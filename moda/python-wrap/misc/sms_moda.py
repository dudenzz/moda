        ...
        for k, front in enumerate(fronts):
            ...
            if len(survivors) + len(front) > n_survive:
                ...
                while len(survivors) + len(front) > n_survive:
                    ...
                    r = solver.Solve(ds,params) # note: Solve actually allocates and deallocates memory - this is a potential problem for small problems (such as 2-objective); additionally python enforces slow and safe memory management (such 
                                              # as shared_ptrs) or the use of built-in garbage collector and no memory management at all (something to be considered, shared_ptrs are slow)
                    k = r[2] #3 is minimum contributor index (0 is minimum contribution, 1 is maximum contribution, 2 in min ctr idx, 3 is max contr idx, 4 is elapsed time)
                    front = np.delete(front, k) # I actually don't know why does it work, front is not an nd.array (it is a dedicated class), if it was, the above would be trivial to solve - perhaps there are helper functions in the pymoo library
            # extend the survivors by all or selected individuals
            survivors.extend(front)

        #print(ds_time, solve_time)
...
class SMSEMOA_MODA(GeneticAlgorithm):
...
parse_doc_string(SMSEMOA_MODA.__init__)
