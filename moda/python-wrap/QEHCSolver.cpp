...
namespace moda {
	QEHCResult* QEHCSolver::Solve(DataSet* problem, QEHCParameters parameters)
    {
		...
		r_prim = (solveQEHC(currentlySolvedProblem->points, currentSettings->nPoints, parameters.SearchSubject, problem->getNadir(), parameters.sort, parameters.shuffle, parameters.offset, parameters.maxlevel, parameters.iterationsLimit));
